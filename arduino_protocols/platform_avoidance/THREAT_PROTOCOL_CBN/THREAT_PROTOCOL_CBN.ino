...
int total_cs_number = 9;          // NUMBER OF CS 
...
void setup() {
  ...
  Serial.println("PLATFORM AVOIDANCE PROTOCOL");
  ...
  pinMode(13, OUTPUT); // Signal the arduino 01 (AR01) to log lever presses
...
void loop() {
  ...
  if (switchstate == HIGH) { 
    ...
    // CUE SECOND ARDUINO TO LOG LEVER PRESSES
    digitalWrite(13, HIGH);
    delay(1000);
    digitalWrite(13, LOW);
    
    Serial.println("NEW EXPERIMENT: PLATFORM AVOIDANCE");
    delay(1000);
  ...
  // start infinite to simulate end of trial
  while (true) {
  };
}
